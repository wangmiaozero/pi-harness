/**
 * Atomic, crash-safe file I/O for the runtime's own stores.
 *
 * Ported from the Electron main process `services/storage.ts` so runtime-side
 * JSON stores share the exact same on-disk behavior (write path: temp sibling
 * → fsync → rename; per-path write queues). File compatibility with the
 * Electron userData stores is the contract — both hosts read and write the
 * same files.
 *
...
/**
 * A small cached JSON store for runtime state (runs, checkpoints, policy...).
 * Reads lazily, writes atomically, and never holds secrets in plaintext
 * beyond the process.
 */
export class JsonStore<T extends object> {
  ...
  async read(): Promise<T> {
    ...
    try {
      ...
      logDiagnostic(`store read failed (${this.filePath}), using defaults: ${err instanceof Error ? err.message : String(err)}`)
