/**
 * Harness Run Repository (runtime port).
 *
 ...
 * concrete database. The default implementation is an atomic JSON store with
 * retention pruning, reading/writing the exact same `harness-runs.json` the
 * Electron main process uses (§37 data compatibility).
 */

import type { JsonStore } from '../../support/json-store.js'
import { logDiagnostic } from '../../transport/jsonl.js'
import type { HarnessRun, HarnessRunTreeNode, HarnessRunRelation } from '../types.js'
...
export async function withRepositoryLog<T>(operation: string, task: () => Promise<T>): Promise<T | null> {
  try {
    ...
    logDiagnostic(`run repository ${operation} failed: ${error instanceof Error ? error.message : String(error)}`)
    return null
  }
}
