/**
 * Harness Policy Engine (runtime port).
 *
 ...
 * Decisions are pure; enforcement lives at the tool boundary
 * (policy-tool-guard.ts).
 *
 * Sidecar adaptation: the Electron BrowserWindow/dialog confirmation is
 * stripped. The runtime has no window, so an `ask` decision resolves to the
 * safe answer — deny — exactly mirroring the Electron rule for a missing
 * window. Decisions still surface through `onDecision` listeners so the host
 * UI can display them.
 */
...
export class PolicyEngine {
  ...
  /**
   * Resolve an `ask` decision. The sidecar runtime has no window, so the
   * safe answer is deny — the control plane never silently escalates.
   * The decision is reported so the host UI can surface it.
   */
  async resolveAsk(
    sessionId: string,
    target: string,
  ...
  report(report: HarnessPolicyDecisionReport): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        logDiagnostic(
          `policy decision listener failed: ${error instanceof Error ? error.message : String(error)}`
        )
      }
    }
  }
