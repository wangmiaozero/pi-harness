/**
 * Policy enforcement at the Pi tool boundary (runtime port).
 *
 ...
 * - `ask`  → no window in the sidecar — resolves to deny (see PolicyEngine).
...
import type { HarnessPolicyDomain } from '../types.js'
import type { PolicyEngine, PolicyEvaluation } from './policy-engine.js'
import { normalizeCommand } from './policy-defaults.js'
...
export function wrapPolicyGuardedTools(
  ...
  const tools = session.getAllTools() as unknown as Array<
