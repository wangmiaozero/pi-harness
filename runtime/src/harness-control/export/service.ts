/**
 * Run export & debug bundle (runtime port).
 *
 * The sidecar never writes user-chosen paths — it renders secret-redacted
 * content and returns it. The Tauri host owns the save dialog / filesystem
 * capability (task/14 §16).
 */

import os from 'node:os'
import { redactSecretText, redactSecrets } from '../../support/secrets.js'
import type {
  HarnessArtifact,
  HarnessDiagnostic,
  HarnessEvaluation,
...
export class RunExportService {
  ...
  async exportRun(
    ...
    return {
      format,
      path: `run-${sanitize(defaultName || payload.run.id)}.${ext}`,
      cancelled: false,
      content
    }
  }
