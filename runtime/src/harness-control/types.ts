/**
 * Runtime-local Harness DTOs.
 *
 * Verbatim port of `src/shared/types/harness.ts`. The runtime sidecar cannot
 * import `@shared/*` (self-contained, zero-dep), so this file is the runtime's
 * authoritative copy of the wire DTOs. These types ARE the data-compatibility
 * contract with the Electron main process and the persisted userData stores —
 * any change here must be mirrored in `src/shared/types/harness.ts`.

 * Ported as part of Phase 3 (Harness Control Plane + Orchestration in the
 * Node runtime, task/14 §4). No Pi internals are redefined here — Pi's own
 * SDK types stay in `../pi/types.js`.
 */

// ---------------------------------------------------------------------------
// Harness session-level DTOs (already mirrored by the phase-2 harness service
// for state/tools/stats; re-declared here so the control plane is complete).
// ---------------------------------------------------------------------------
...
/** §16: the runtime renders export content (secret-redacted); the host saves it. */
export interface HarnessExportResult {
  format: 'json' | 'markdown'
  path: string
  cancelled: boolean
  /** Content the host should write to the user-chosen file. */
  content: string
}
