...
import { HarnessError } from '../harness-control/harness-error.js'
import { detectCycles } from './dependency-resolver.js'
import {
  OrchestrationStore,
  newOrchestrationId,
  newTaskId
} from './store.js'
import { AgentManager, type AgentDefinition } from './agents.js'
import { TeamService } from './teams.js'
import { HandoffService } from './handoffs.js'
import { ConflictService } from './conflicts.js'
import { TaskScheduler, type DispatchCandidate, type SchedulingSnapshot } from './scheduler.js'
import { TaskContextBuilder, parseReviewVerdict } from './task-context.js'
...
n * even if a dispatch is still legitimately running (no event will ever
  ...
  private async reconcile(orchestrationId: string): Promise<void> {
    ...
    if (
      ...
      // Persist the evaluation before flipping the orchestration to a
      // terminal status so readers that wait on `completed` always see it.
