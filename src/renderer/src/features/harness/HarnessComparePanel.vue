...
function runOption(run: HarnessRun): string {
  return `${run.prompt.slice(0, 50) || t('workspace.harnessRunUntitled')} · ${t(`workspace.harnessRunStatus.${run.status}`)}`
}
...
<template>
  <section class="harness-card" data-testid="harness-compare-panel">
    ...
    <div class="mt-3 flex flex-wrap items-center gap-2">
      ...
      >
        <option value="" disabled>{{ t('workspace.harnessCompareRunA') }}</option>
        <option v-for="run in runs" :key="run.id" :value="run.id">{{ runOption(run) }}</option>
      </select>
      ...
      >
        <option value="" disabled>{{ t('workspace.harnessCompareRunB') }}</option>
        <option v-for="run in runs" :key="run.id" :value="run.id">{{ runOption(run) }}</option>
      </select>
      <button
        type="button"
    ...
    <p
      v-if="compareError"
      role="alert"
      class="mt-2 text-[11.5px] text-[var(--error)]"
    >
      {{ compareError }}
    </p>
    ...
    <template v-if="comparison">
      <div class="mt-4 overflow-x-auto" data-testid="harness-compare-metrics">
        <table class="w-full min-w-[30rem] border-collapse text-[11.5px]">
          <thead>
            <tr class="border-b border-[var(--border-subtle)] text-left text-[10.5px] uppercase tracking-wide text-[var(--text-tertiary)]">
          ...
          <tbody>
            ...
            >
              <td class="py-1.5 pr-2 text-[var(--text-secondary)]">
                {{ t(`workspace.harnessCompareMetric_${metric.id.replace(/[A-Z]/g, (c) => '_' + c.toLowerCase())}`) }}
              </td>
              ...
              <td class="py-1.5 tabular-nums" :class="toneClass[metric.tone]">
                <span class="inline-flex items-center gap-1">
                  <component
                    :is="metric.tone === 'better' ? TrendingDown : metric.tone === 'worse' ? TrendingUp : Minus"
                    class="size-3"
                  />
                  {{ metric.delta ?? '—' }}
                </span>
      ...
      <div v-if="comparison.findings.length" class="mt-4">
        ...
        <ul class="space-y-0.5 text-[11px]">
          <li
            ...
            :class="finding.severity === 'regression' ? 'text-[var(--error)]' : finding.severity === 'improvement' ? 'text-[var(--success)]' : 'text-[var(--text-secondary)]'"
          >
      ...
      <div v-for="section in comparison.diffs" :key="section.id" class="mt-4">
        ...
        <div class="max-h-56 overflow-auto rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-2 font-mono text-[10.5px] leading-relaxed">
          <p
